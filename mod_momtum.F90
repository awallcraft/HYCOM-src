...
! ---   assume margin<nblk
        ...
        do i=1-margin,ii+margin
          if (SEA_U) then
            if     (shaved.lt.0) then !no sidewall
              wgtja(i,j)=0.0
              wgtjb(i,j)=0.0
            else  !sidewall w.r.t. partial cells
              wgtja(i,j)=max(0.0,min(1.0,(pu(i,j,k+1)-dpthup(i,ja)) &
                        /max(pu(i,j,k+1)-pu(i,j,k),epsil)))
              wgtjb(i,j)=max(0.0,min(1.0,(pu(i,j,k+1)-dpthup(i,jb)) &
                        /max(pu(i,j,k+1)-pu(i,j,k),epsil)))
            endif
...
! ---       assume margin<nblk
            ...
            if     (shaved.lt.0) then !no sidewall
              ...
              wgtia(i,j)=max(0.0,min(1.0,(pv(i,j,k+1)-dpthvp(ia,j)) &
                        /max(pv(i,j,k+1)-pv(i,j,k),epsil)))
              wgtib(i,j)=max(0.0,min(1.0,(pv(i,j,k+1)-dpthvp(ib,j)) &
                        /max(pv(i,j,k+1)-pv(i,j,k),epsil)))
            endif
...
! ---   compare two model runs.
        ...
        call pipe_compare_sym1( dpthup,iu,text)
...
!
        ...
        call pipe_compare_sym1( dpthvp,iv,text)
